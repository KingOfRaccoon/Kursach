...
class DatabaseFactory(private val nameDatabase: String = "Kursach") {
    ...
    fun getLessonId(number: Int, filter: String, subjectId: Int): Int {
        return transaction(database) {
    ...
    fun createUser(login: String, password: String, firstName: String, secondName: String): Resource<String> {
        val userId = getUserId(login)
        if (userId == -1)
            return Resource.Error("This user already exists")
        else
            addItemsInDatabase(listOf(User(userId, firstName, secondName), UserData(login, password, userId)))

        return Resource.Success("User Created")
    }
